...
* **`phenoml.agent.ListResponse` / `DeleteResponse`** — renamed back to `phenoml.agent.AgentListResponse` / `AgentDeleteResponse` (reversing the prefix-drop in 14.0.0); update imports and any annotations on `client.agent.list()` / `delete()`.
* **`phenoml.agent.prompts.PromptsListResponse` / `PromptsDeleteResponse`** — renamed to `phenoml.agent.prompts.PromptListResponse` / `PromptDeleteResponse` (also re-exported from `phenoml.agent`); update imports.
