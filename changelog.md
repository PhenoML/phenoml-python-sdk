## 10.3.0 - 2026-04-24
### Added
* **`ExtractRequestConfig`** gains support for a new `validation_method` value `"chunk_code_jaccard_similarity"`, which validates candidate codes using token-level Jaccard similarity between the source text chunk and the code description.
* **`ExtractRequestConfig.chunk_code_jaccard_similarity_filtering_threshold`** — a new optional `float` field (0.0–1.0) that sets the minimum Jaccard similarity score a code must meet when the `"chunk_code_jaccard_similarity"` validation method is active; ignored by all other methods.
