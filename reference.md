...
<details><summary><code>client.construe.codes.<a href="src/phenoml/construe/codes/client.py">phenocr</a>(...) -> ExtractCodesResult</code></summary>
<dl>
<dd>
...
<details><summary><code>client.lang2fhir_batch.<a href="src/phenoml/lang2fhir_batch/client.py">cancel</a>(...) -> BatchJob</code></summary>
<dl>
<dd>
...
This is the way to terminate a job that was created but never finalized —
an empty job with no items most acutely — which `finalize` refuses and
which would otherwise hold one of the instance's 4 active-job slots until
it is aged out at 36 hours.

Cancel is a soft transition, not a delete: the job record and any results
already produced stay readable for the normal retention window, the same
as a `completed` or `failed` job. Item statuses freeze at cancellation —
items that already succeeded or failed keep that outcome and their
results, while any still `pending` or `processing` stay in that state and
are not processed further, so a canceled job's `counts` may show
unfinished items that never resolve.

Canceling a job that has already finished (`completed`, `failed`, or
`canceled`) is a `409`.
</dd>
</dl>
</dd>
